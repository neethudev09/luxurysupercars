...
interface StickyEnquireBarProps {
  car: Car;
  /** CSS selector of the section to scroll PAST before the bar appears.
   *  The bar shows once this element's bottom edge leaves the top of the
   *  viewport (user has scrolled past it). */
  triggerSelector?: string;
}

export default function StickyEnquireBar({
  car,
  triggerSelector = "#specs",
}: StickyEnquireBarProps) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // Bar appears once the section's bottom has scrolled above the viewport
